...
namespace ImGui {
  ...
  inline static tm GetCurrentDate() {
    ...
    tm result;
#ifdef _WIN32
    localtime_s(&result, &now); // Windows-specific safe version
#else
    localtime_r(&now, &result); // POSIX safe version
#endif
    return result;
  }
  ...
  inline static void RecalculateDateDependentFields(tm& date) {
    date.tm_isdst = -1;   // This tries to detect daylight saving time
    time_t tmp = mktime(&date);
     tm result;

    #ifdef _WIN32
        localtime_s(&result, &tmp); // Windows-specific safe version
    #else
        localtime_r(&tmp, &result); // POSIX safe version
    #endif
...
#ifdef _WIN32
          strcpy_s(monthName, 32, fallbacks[i]); // Windows: safer strcpy with buffer size
#else
          strncpy(monthName, fallbacks[i], 31);  // POSIX: safer strncpy with buffer size
          monthName[31] = '\0';                  // Ensure null-termination
#endif
        }
